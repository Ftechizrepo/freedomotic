...
public final class Autodiscovery extends AbstractConsumer {
     ...
     *
     * @param command contains needed properties for parameters to call
     ...
     * @throws com.freedomotic.exceptions.RepositoryException if it is not
     * possible to retrieve the requested information
     */
    protected EnvObjectLogic join(String clazz, String name, String protocol, String address, boolean allowClones)
        ...
        if (thingAlreadyExists(protocol, address)) {
            LOG.info("A thing with protocol \"{}\" and address \"{}\" already exists in the environment. Autodiscovery exits without changes",
                    protocol, address);
            return null;
        }
        ...
        if (!allowClones && thingAlreadyExists(name)) {
            LOG.info("A thing with name \"{}\" already exists in the environment. Autodiscovery exits without changes because property 'autodiscovery.allow-clones' property is ''{}'' for the received command",
                    name, false);
            return null;
        }
     ...
     * @param loaded the target things
     */
    ...
    /**
     * Maps object behaviors to hardware triggers by calling
     * {@link EnvObjectLogic#addTriggerMapping(Trigger, String)}
     *
     * @param tuple Object with trigger name
     * @param environmentLogic Environment logic on which to map trigger and
     * behavior
     */
    ...
    /**
     * Calls {@link EnvObjectLogic#setAction(String, Command)} with command name
     * from <code>tuple</code>
     *
